...
RSpec.describe Public::GameOwnershipsController do
  before do
  ...
  describe 'POST #create' do
    it 'adds game to user' do
      post_json :create, access_token: @users[0].session.access_token,
        model: {user_id: @users[0].id, game_id: @games[3].id, nickname: 'nickname0'}
    ...
    it 'cannot add game multiple times' do
      post_json :create, access_token: @users[0].session.access_token,
        model: {user_id: @users[0].id, game_id: @games[0].id, nickname: 'nickname0'}
    ...
    it 'cannot add game to other user' do
      post_json :create, access_token: @users[0].session.access_token,
        model: {user_id: @users[1].id, game_id: @games[0].id, nickname: 'nickname0'}
  ...
  describe 'PATCH #update' do
    it "updates user's nickname" do
      patch_json :update, user_id: @users[0].id, id: @users[0].game_ownerships[0].id,
        access_token: @users[0].session.access_token, model: {nickname: 'nyk'}
      expect(response.status).to eql(200)
